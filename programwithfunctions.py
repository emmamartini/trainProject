...
def new_user(userInfo):
    ...
    cur.execute('SELECT Email, PhoneNumber FROM passenger WHERE Email = ? OR PhoneNumber = ?', (userInfo[2], userInfo[4]))
            ...
            if yourPhoneNumber == info[5]:
                ...
                if check_password(info[4], yourPassword) and info[6] == 0:
                    ...
                    if activationAnswer == "Yes":
                        ...
                        print("Thank you! Your account has been activated! Please log in again")
                        conn.commit()
                    returning()
                    break
...
def user_dashboard(userInfo):
    ...
    while True:
        ...
        cur.execute("SELECT Active FROM Passenger WHERE PassengerId = ?", (userInfo[0],))
...
def see_train_subscriptions(passengerId):
    conn = connect_to_database()
    cur = conn.cursor()
    cur.execute('SELECT * FROM Subscription WHERE PassengerId = ? AND Active = 1', (passengerId,))
...
def train_existence(subscription):
        ...
        subscriptionList = []
        print(subscription)
        cur.execute('SELECT StationSignature FROM Station where StationId = ?', (subscription[1],))
...
def delete_subscription(passengerId):
    ...
    try:
        cur.execute('SELECT SubscriptionId FROM Subscription WHERE Active = 1 AND PassengerId = ?', (passengerId,))
        ...
        if len(allSubscriptionsQuery) == 1:
            ...
            print("Thank you! You only had one active subscription and it has now been unactivated!")
            conn.commit()
            returning()
            return
    except:
...
def update_information(passengerId):
    ...
    while True:
        ...
        if updateNumber==1:
            ...
            cur.execute("UPDATE passenger SET FirstName = ? WHERE PassengerId = ?", (updateFirstName, passengerId))
        ...
        if updateNumber==2:
            ...
            cur.execute("UPDATE passenger SET LastName = ? WHERE PassengerId = ?", (updateLastName, passengerId))
        ...
        if updateNumber==3:
            while True:
                ...
                if existingEmail:
                    ...
                    break
            cur.execute("UPDATE passenger SET Email = ? WHERE PassengerId = ?", (updateEmail, passengerId))
        ...
        if updateNumber==5:
            while True:
                ...
                if existingPhoneNumber:
                    ...
                    break
            cur.execute("UPDATE passenger SET PhoneNumber = ? WHERE PassengerId = ?", (updatePhoneNumber, passengerId))
        ...
        if updateNumber==6:
            ...
            if deleteOrNot == "Yes":
                cur.execute("UPDATE Passenger SET Active = 0 WHERE PassengerId = ?", (passengerId,))
